var RTVBox = function(){
  ...
  ISOBox.prototype._boxParsers['styp'] = function() {
    this.major_brand = this._readString(4);
    this.minor_versions = this._readUint(32);
    this.compatible_brands = [];
    if(this._raw.byteLength > 24 ){
      console.log("STYP DATA ---" ,this );
      var codeArr = [105,115,111,54];
      for(var i = 24 ; i < 28; i++){
         this._raw.setUint8(i, codeArr[(i-24)]);
        console.log("STYP char - ", String.fromCharCode( this._raw.getUint8(i) ) );
      }
    }
  ...
  ISOBox.prototype._boxParsers['mfhd'] = function() {
    this._parseFullBox();
    //RTVDash.tfdtOffset.push('MFHD');
    //RTV objects
    this.mfhdRTV_EPTimeOffset = this._cursor.offset - this._raw.byteOffset;
    this._raw.setUint32( 12, RTVDash.index );
  ...
  ISOBox.prototype._boxParsers['sidx'] = function() {
    ...
    //RTV objects
  if (this.version == 0) {
      this._raw.setUint32( 20,RTVDash.time );
    }else{
      //TODO add uint64
    //  console.log("RTV-TIME = ", RTVDash.time);
      this._raw.setUint32( 24,RTVDash.time );//need to add64 bit bi uint32 (XXXX XXXX)
    }
  ...
  ISOBox.prototype._boxParsers['tfdt'] = function() {
    ...
    //RTVDash.tfdtOffset.push((this._cursor.offset - this._raw.byteOffset) + 4 );
    this._parseFullBox();

    //RTV objects
    this.tfdtRaw = this._raw;
    this.RTV_version = this.version;


    if(this.RTV_version == 1){
      //TODO add uint64
      //console.log("RTV-TIME = ", RTVDash.time);
      this._raw.setUint32( (this._cursor.offset - this._raw.byteOffset) + 4, RTVDash.time );
    }else{

      this._raw.setUint32( (this._cursor.offset - this._raw.byteOffset), RTVDash.time );
    }
